...
export default function PropiedadesPage() {
  ...
  const generateFichaPDF = (p: Propiedad) => {
    try {
      ...
      doc.save(`Ficha_${p.codigo || 'PROP'}_${nombre.replace(/\s+/g, '_')}.pdf`)
    } catch (err) {
                ...
                <tr key={p.id} className="hover:bg-white/5 transition-colors" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => { setViewRecord(p); setGalleryIndex(0) }} className="px-2 py-1 rounded-lg text-xs font-medium hover:opacity-90" style={{ background: 'rgba(4,120,87,0.9)', border: '1px solid rgba(4,120,87,1)', color: '#fff' }}>Ver</button>
